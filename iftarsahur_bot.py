...
sahib: str = environ.get("sahib")
kanal: str = environ.get("kanal")
...
keyboard = InlineKeyboardMarkup([
    [
        InlineKeyboardButton("Kanal 📚", url=f"https://t.me/{kanal}")
    ],
    [
        InlineKeyboardButton("🇦🇿 Developer 🇦🇿 ", url=f"https://t.me/{sahib}")
    ],
    [
        InlineKeyboardButton("Gruba Ekle  🎉", url=f"http://t.me/{BOT_USERNAME}?startgroup=new")
    ]
])
...
"""
@app.on_message(f.command(["start",f"start@{BOT_USERNAME}"]) & f.private)
async def priv_start(c:Client, m:Message):
    await c.send_message(m.chat.id, texts.text.START.format(m.from_user.mention), reply_markup=keyboard)

"""
GROUP /start MESSAGE
"""
@app.on_message(f.command(["start",f"start@{BOT_USERNAME}"]) & ~f.private)
async def group_start(c:Client, m:Message):
    await c.send_message(m.chat.id,texts.text.START.format(m.from_user.mention), reply_markup=keyboard)

    sayi = None
    for i in data.find({"$and": [{"cgc": True}]}):
        sayi = i["count_group_chat"]
    myquery = {"count_group_chat": sayi}
    sayi += 1
    newvalues = {"$set": {"count_group_chat": sayi}}
